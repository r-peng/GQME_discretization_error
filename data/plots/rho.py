...
def plot(U,dt,ax,c,l):
    ...
    ax.plot(x,rho[:,0].real,linestyle='-',color=c,label=l)
    ax.plot(x,rho[:,1].real,linestyle=':',color=c)
...
for dt in np.array([.01,.1,.2]):
    ...
    ax.plot(x,rho_ref[:,0,0].real,linestyle='-',color='k',label='HEOM')
    ax.plot(x,rho_ref[:,0,1].real,linestyle=':',color='k')
    ...
    ax.set_xlim(0,10)
    ax.legend()
    fig.subplots_adjust(left=0.16, bottom=0.15, right=0.97, top=0.98)
