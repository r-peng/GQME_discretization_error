...
class environment_slice:
    ...
    def compute(Ai, Bi, Hi = None):
        if not isinstance(Hi,np.ndarray):
            return environment_slice.compute_matrix_element(Ai, Bi, Hi)
        else:
            return environment_slice.compute_overlap(Ai, Bi)
...
def one_site_linear_operator(Li, Hi, Ri, coeff = 1.0):    
    def f(v):
        av = None
        if v.ndim == 1:
...
def one_site_linear_operator_trace(Li, Hi, Ri, coeff=1.0):
...
def one_site_linear_operator_transpose(Li, Hi, Ri, coeff = 1.0):
    def f(v):
        av = None
        if v.ndim == 1:
            av = coeff*v.reshape((1, Li.shape[2], Hi.shape[2], Ri.shape[2]))
...
def two_site_linear_operator(Li, Hi, Hir, Ri, coeff = 1.0):
    def f(v):
        av = None
        if v.ndim == 1:
...
def two_site_linear_operator_transpose(Li, Hi, Hir, Ri, coeff = 1.0):
    def f(v):
        av = None
        if v.ndim == 1:
            av = coeff*v.reshape((1, Li.shape[2], Hi.shape[2], Hir.shape[2], Ri.shape[2]))
...
def one_site_environment(Li, Ti, Ri, coeff = 1.0, Hi = None):
    if not isinstance(Hi, np.ndarray):
...
def two_site_environment(Li, Ti, Tir, Ri, coeff = 1.0, Hi = None, Hir = None):
