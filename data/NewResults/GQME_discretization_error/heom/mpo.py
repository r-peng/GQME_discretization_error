...
class mpo:
    ...
    def __init__(self, chi = None, d = None, N = None, chil : int = 1, chir : int = 1, dtype = np.double, init = 'zeros', do_orthog=False):
        self.dtype = dtype 
    ...
    def sanitise(self):
        ...
        if not self.is_valid():
            for bi in range(self.nbonds()):
                chil = self._tensors[bi].shape[3]
                chir = self._tensors[bi+1].shape[0]
                if not chil == chir:
                    chi = max(chil, chir)
                    if(chi > chil):
                        npad = chi - chil
                        self._tensors[bi] = np.pad(self._tensors[i], ((0,0), (0, 0), (0, 0), (0,npad) ), 'constant', constant_values=(0))
                    else:
                        npad = chi - chir
                        self._tensors[i+1] = np.pad(self._tensors[i+1], ((0,npad), (0, 0), (0, 0), (0,0) ), 'constant', constant_values=(0))
            self._is_valid = True
    ...
    def swap_bond_indices(self, bi, dir='right', tol=None, nbond=None):
        ...
        M = np.transpose(self.contract_bond(bi), axes=[0, 3, 4, 1, 2, 5])
    ...
    def contract_bond(self, bi):
        ...
        return np.tensordot(self._tensors[bi], self._tensors[bi+1], axes=([3], [0]))
    ...
    def decompose_two_site(self, M, bi, dir = 'right', tol = None, nbond = None):
        ...
        Mm = M.reshape((dims[0]*dims[1]*dims[2], dims[3]*dims[4]*dims[5]))
        ...
        if dir == 'right':
            ...
            self._tensors[il] = Q.reshape( (dims[0], dims[1], dims[2], nsvd))
            self._tensors[ir] = R.reshape( (nsvd, dims[3], dims[4], dims[5]))
            ...
            R = Q @ S
            self._tensors[il] =  R.reshape( (dims[0], dims[1], dims[2], nsvd))
            self._tensors[ir] = Vh.reshape( (nsvd, dims[3], dims[4], dims[5]))
    ...
    def apply_MPO_node(Mt, nt, order = 'mpo', dir='up'):
        ...
        if(order == 'mpo'):
            if(dir == 'up'):
                #ret = np.transpose(np.tensordot(Mt, nt, axes=([2], [1])), axes=[0, 3, 1, 4, 2, 5])
                ret = np.einsum('aijb, mjkn -> amikbn', Mt, nt)
            else:
                #ret = np.transpose(np.tensordot(nt, Mt, axes=([2], [1])), axes=[0, 3, 1, 4, 2, 5])
                ret = np.einsum('aijb, mkjn -> amkibn', Mt, nt)
        else:
            if(dir == 'up'):
                #ret = np.transpose(np.tensordot(Mt, nt, axes=([3], [1])), axes=[0, 3, 1, 4, 2, 5])
                ret = np.einsum('aibj, mjkn -> amikbn', Mt, nt)
            else:
                #ret = np.transpose(np.tensordot(nt, Mt, axes=([2], [1])), axes=[0, 3, 1, 5, 2, 4])
    ...
    def _apply_nsite(self, M, inds, method='naive', tol = None, nbond = None, optol = None, dir='up'):
        ...
        if method == "naive":
            ...
            for ind in range(i, j+1):
                if(ind == inds[c]):
                    self._tensors[ind] = mpo.apply_MPO_node(Mp[c], self._tensors[ind], order='clockwise', dir=dir)
                    nbd = Mp[c].shape[2]
                    c = c+1
                else:
                    d = self._tensors[ind].shape
                    self._tensors[ind] = mpo.apply_MPO_node(identity_pad_mpo(nbd, d[1], order='clockwise'), self._tensors[ind], order='clockwise', dir=dir)
    ...
    def apply_MPO_naive(self, M, tol = None, nbond = None, dir='up'):
        ...
        self.shift_orthogonality(-1)#, tol, nbond)
    ...
    def apply_MPO_density_matrix(self, M, tol=None, nbond=None):
        check_compatible(self, M, mpo, mpo)
        return 
    ...
    def apply_MPO(self, M, method="naive", tol = None, nbond = None):
        if(len(self) == 1):
            ...
            elif method == "densitymatrix":
                self.apply_MPO_density_matrix(M, tol=tol, nbond=nbond)
    ...
    def __add__(self, other):
        ...
        for i in range(1, len(self)-1):
            csl = self.bond_dimension(i-1)
            csr = self.bond_dimension(i)
            ret[i][0:csl, :, :, 0:csr] = self[i]
            ret[i][csl:chis[i-1], :, :, csr:chis[i]] = other[i]
